...
export const TankGauge: React.FC<TankGaugeProps> = ({
  ...
  const getColor = () => {
    if (waterLevel >= dangerThreshold) {
      return "bg-gradient-to-t from-red-600 to-red-500 animate-pulse"; // Danger state with pulse
    }
    if (waterLevel >= 85 || waterLevel <= 10) {
      return "bg-gradient-to-t from-amber-600 to-amber-500"; // Near danger
    }
    ...
    <div className="flex flex-col items-center space-y-3">
      <div className="relative w-36 h-64 bg-surface-2 rounded-2xl overflow-hidden border-4 border-border-strong shadow-inner transition-colors">
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-2 pointer-events-none z-10 opacity-40">
          <div className="border-b border-text-tertiary text-[9px] font-mono text-text-tertiary">100%</div>
          <div className="border-b border-text-tertiary text-[9px] font-mono text-text-tertiary">75%</div>
          <div className="border-b border-text-tertiary text-[9px] font-mono text-text-tertiary">50%</div>
          <div className="border-b border-text-tertiary text-[9px] font-mono text-text-tertiary">25%</div>
        ...
        <div
          className="absolute left-0 right-0 h-0.5 bg-red-500 border-t border-red-600 z-20 shadow-xs flex items-center justify-end pr-1"
          style={{ bottom: `${dangerThreshold}%` }}
        >
          <span className="text-[8px] font-mono font-bold text-red-600 bg-surface-1/90 px-1 rounded shadow-xs">
