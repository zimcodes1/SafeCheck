        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1">
              Time-series logs of water tank levels and actuator states polled via Modbus.
            </p>
          </div>
          <DataExportButton
            data={readings}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              ...
              <div className="grid grid-cols-4 gap-2 text-center text-xs">
                <div className="bg-surface-2 px-3 py-1.5 rounded-lg border border-border-subtle">
                  <span className="text-text-tertiary block text-[10px] uppercase font-semibold">Latest</span>
                  <span className="font-bold text-text-primary text-sm">{latestLevel.toFixed(1)}%</span>
                </div>
                <div className="bg-surface-2 px-3 py-1.5 rounded-lg border border-border-subtle">
                  <span className="text-text-tertiary block text-[10px] uppercase font-semibold">Average</span>
                  <span className="font-bold text-text-primary text-sm">{avgLevel.toFixed(1)}%</span>
                </div>
                <div className="bg-surface-2 px-3 py-1.5 rounded-lg border border-border-subtle">
                  <span className="text-text-tertiary block text-[10px] uppercase font-semibold">Min</span>
                  <span className="font-bold text-text-primary text-sm">{minLevel.toFixed(1)}%</span>
                </div>
                <div className="bg-surface-2 px-3 py-1.5 rounded-lg border border-border-subtle">
                  <span className="text-text-tertiary block text-[10px] uppercase font-semibold">Max</span>
                  <span className="font-bold text-text-primary text-sm">{maxLevel.toFixed(1)}%</span>
                </div>
              </div>
            </div>
                ...
                <table className="min-w-full divide-y divide-border-subtle">
                  ...
                  <tbody className="bg-surface-1 divide-y divide-border-subtle">
                    {readings.map((reading) => (
                      <tr key={reading.id} className="hover:bg-surface-2/60 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-text-primary">
                          <div className="font-medium">{getRelativeTime(reading.timestamp)}</div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {reading.pump_state ? (
                            <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">
                              RUNNING
                            </span>
                          ) : (
                            <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-slate-500/10 text-slate-500 border border-slate-500/20">
                              STOPPED
                            </span>
                          )}
                        </td>
