...
**Valid command, wrong moment** — watches the current Plant state, waits for a moment where the water level is already high (near the danger threshold) **and** the valve is closed, then deliberately sends "close valve" while the pump is running (or, if the valve is already closed and the tank is high, sends "pump on") — a perfectly normal command in isolation, but combined with a tank that's already nearly full and has nowhere to send incoming water, creates the pressure-buildup danger condition: a pump switched on against a closed valve, exactly as named in the track brief. The water-level condition matters — pump on with valve closed at low or mid tank level is completely normal filling behavior, not an attack.
...
**Layer 2 — State-machine validity.** This is the core of the whole project. Maintain a simple lookup: given the Plant's state *right before* a command arrives, is the incoming command safe? Pump on with the valve closed is completely normal — that's how the tank fills in the first place. The danger is specific: pump running (or about to start) **with the valve closed and the water level already near the danger threshold**, since the pump is then pushing against a dead end with nowhere left for the water to go. So "pump on" is unsafe when the valve is closed *and* the tank is already high; "close valve" is unsafe when the pump is running *and* the tank is already high. Both halves of the rule need the water-level condition — without it, ordinary filling would be flagged as an attack. This isn't machine learning; it's a table of (current state → incoming command → safe or unsafe) that you define by hand from the physics you already know. This single layer is what catches the "valid command, wrong moment" attack, because it's evaluating context, not the command in isolation.
